...
class Booking:
    ...
    def revenue_by_status():
        query = f"""
            ...
            SELECT 
                ...
                db.avg_price_per_night_eur as actual_paid_price,
                db.bookings_count,
                CASE 
    ...
    def occupancy_rate():
        query = f"""
            SELECT 
                ...
                hp.price_per_night as price_eur,
                -- Tendencias con window functions
    ...
    def optimized_revenue():
        query = f"""
            ...
            -- Revenue potencial con nuevo precio
            ROUND(
                CASE 
                    WHEN (i.allotment_total - i.allotment_sold) <= 2 
...
class Flights:
    ...
    def flight_popularity():
        ...
        SELECT 
            ...
            avg(rp.total_flights_taken),
            avg(rp.avg_customer_spend) as avg_customer_spend_eur,
            avg(rp.booking_rank)
    ...
    def flight_inventory():
        ...
        WITH inventory_status AS (
            SELECT 
                ...
                round(AVG(p.total_fare) OVER (PARTITION BY f.flight_id),2) as avg_fare_for_flight,
                round(AVG(p.total_fare) OVER (PARTITION BY f.destination),2) as avg_fare_for_destination
            FROM travel.flights f
        ...
        SELECT 
            ...
            ia.avg_fare_for_flight, sum(ia.seats_available) seats_available, 
            ia.avg_fare_for_destination, 
            SUM(CASE WHEN total_fare < avg_fare_for_flight THEN 1 ELSE 0 END) 
                as below_avg_price,
            SUM(CASE WHEN total_fare > avg_fare_for_flight THEN 1 ELSE 0 END) 
                as above_avg_price,
            ROUND(
                SUM(CASE WHEN total_fare < avg_fare_for_flight THEN 1 ELSE 0 END)::DECIMAL 
                / COUNT(DISTINCT ia.flight_id)*10, 2) as percent_below_avg
        FROM inventory_status as ia
        GROUP BY ia.flight_id, ia.total_fare, ia.avg_fare_for_flight,
        ia.avg_fare_for_destination,ia.origin,
...
class Payments:
    def customer_payments():
        ...
        SELECT 
            ...
            ROUND(AVG(avg_payment_eur), 2) as avg_spent,
    ...
    def payments_overview():
        ...
        COUNT(DISTINCT p.payment_id) as total_payments,
        COUNT(DISTINCT b.booking_id) as total_bookings,
        SUM(p.amount) as total_payments_eur,
        SUM(b.total_amount) as total_bookings_eur,
        ...
        ORDER BY payment_month DESC, total_payments_eur DESC;
        """
        return query
...
class Customers:
    ...
    def customer_flight_preferences():
       ...
       SELECT 
            ...
            MAX(p.total_fare) as max_price,
            PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY total_fare) as median_price,
            ROW_NUMBER() OVER (ORDER BY SUM(p.total_fare) DESC) as popularity_rank,
            b.customer_id
        FROM travel.flights f
        JOIN travel.booking_items b ON f.flight_id = b.flight_id
        JOIN travel.prices p ON p.price_id = b.price_id 
        GROUP BY f.destination, b.customer_id;
        """
    ...
    def customer_class_flight():
        query = f"""
        SELECT 
        count(f.destination), f.destination,
        AVG(p.total_fare) as avg_price,
